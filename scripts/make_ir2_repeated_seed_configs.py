#!/usr/bin/env python3
"""
Create repeated-seed configs for IR2 scratch vs encoder-only experiments.

The goal is to test whether the supervised encoder transfer improvement on IR2
is stable across random seeds.

Generated configs:
    configs/repeated_seeds/ir2_scratch_seed44.yaml
    configs/repeated_seeds/ir2_scratch_seed45.yaml
    configs/repeated_seeds/ir2_scratch_seed46.yaml
    configs/repeated_seeds/ir2_encoder_only_seed44.yaml
    configs/repeated_seeds/ir2_encoder_only_seed45.yaml
    configs/repeated_seeds/ir2_encoder_only_seed46.yaml
"""
...
BASE_SCRATCH_CONFIG = Path("configs/feature_extraction/infrared2_scratch.yaml")
BASE_ENCODER_ONLY_CONFIG = Path("configs/feature_extraction/infrared2_encoder_only.yaml")
...
def make_config(base_path: Path, experiment_name: str, seed: int) -> Path:
    ...
    if "scratch" in experiment_name:
        config = recursively_replace_string(
            config,
            "results/feature_extraction/infrared2_scratch",
            f"results/repeated_seeds/{experiment_name}",
        )
        config = recursively_replace_string(
            config,
            "models/feature_extraction/infrared2_scratch",
            f"models/repeated_seeds/{experiment_name}",
        )
    else:
        config = recursively_replace_string(
            config,
            "results/feature_extraction/infrared2_encoder_only",
            f"results/repeated_seeds/{experiment_name}",
        )
        config = recursively_replace_string(
            config,
            "models/feature_extraction/infrared2_encoder_only",
            f"models/repeated_seeds/{experiment_name}",
        )
...
def main() -> None:
    ...
    for seed in SEEDS:
        generated.append(make_config(BASE_SCRATCH_CONFIG, f"ir2_scratch_seed{seed}", seed))
        generated.append(make_config(BASE_ENCODER_ONLY_CONFIG, f"ir2_encoder_only_seed{seed}", seed))
